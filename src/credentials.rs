...
pub fn private_write(path: &Path, bytes: &[u8]) -> Result<()> {
    ...
    // Close the temporary file handle before replacing the destination.
    //
    // This matters on Windows: persisting an open NamedTempFile can fail
    // while replacing an existing file. TempPath keeps ownership of the
    // temporary path while allowing the underlying file handle to close.
...
pub trait CredentialStore {
    ...
    // true explicitly selects file, false keychain.
    ...
    fn load(&self, reference: &CredentialRef) -> Result<Secret> {
        ...
        let value = match reference {
            CredentialRef::Keyring { account } => {
                keyring::Entry::new("bkpi", account)
                    .and_then(|e| e.get_password())
                    .map_err(|_| {
                        anyhow::anyhow!(
                            "OS credential unavailable; update the integration credential"
                        )
                    })?
            }
            ...
            CredentialRef::File { account } => {
                ...
                fs::read_to_string(self.file(account)?)
                    .map_err(|_| {
                        anyhow::anyhow!(
            ...
            CredentialRef::Env { account } => {
                std::env::var(account).map_err(|_| {
                    anyhow::anyhow!(
                        "Credential environment variable is missing or not Unicode; set it in the BKPI process environment"
                    )
                })?
            }
        };
        ...
        match reference {
            CredentialRef::Keyring { account } => {
                match keyring::Entry::new("bkpi", account).and_then(|e| e.delete_credential()) {
                    ...
                    Err(_) => {
                        bail!("Cannot remove OS credential")
                    }
                }
            }
            ...
            CredentialRef::File { account } => {
                ...
                match fs::remove_file(path) {
                    ...
                    Err(_) => {
                        bail!("Cannot remove credential file")
                    }
                }
            }

            CredentialRef::Env { .. } => {
                // BKPI never changes the process environment.
                Ok(())
            }
        }
    }
...
/// Defense in depth for accidentally pasted credentials
/// in documents/remote text.
